var pokemonList = {                 //pokemon JSON assigned to variable pokemonList
    "pokemon": [{
      "id": 1,
      "num": "001",
      "name": "Bulbasaur",
...
var getpokemonDetails = function(name) {

    var isFound = false                                     //flag isFound set to false

    for (x in pokemonList.pokemon) {                        //iterate pokeon array
      if (pokemonList.pokemon[x].name == name) {            //if iterated name matches entered name
  
        isFound = true                                      //set isFound to true
        var details = JSON.stringify(pokemonList.pokemon[x], null, 2)                          //store pokemon object in var details with pretty print
        document.getElementById("id1").innerText = "The Details of Entered Pokemon  - \n\n" + details         //display pokemon details on html page
        break
...
var getpokemonEvolution = function(name) {

    var anyFound = false                                //flag anyFound set to false
    var pokeArray = []                                  //initialized empty pokeArray

    for (var x in pokemonList.pokemon) {                //iterate pokeon array
  
     if ((pokemonList.pokemon[x]).hasOwnProperty("next_evolution")) {              // check if key "next_evolution" exists for the current iterated pokemon
            for (var y in pokemonList.pokemon[x].next_evolution) {                 //iterate through next_evolution array for each pokemon array
                if (pokemonList.pokemon[x].next_evolution[y].name == name) {       //if iterated name matches entered name
        
                    anyFound = true                                                 //set anyFound flag to true
                    pokeArray.push(pokemonList.pokemon[x].name)                     //push pokemon name to pokeArray
    ...
    if (anyFound == true) {                                         //if anyFound set to true, Display the  pokeArray on the html page
        document.getElementById("id1").innerText = "The Pokemons having name " + name + "  in the next evolution - \n\n" + pokeArray.toString();
    } else {
        alert("No such pokemon found")                             //if no result was found, alert No pokemon found
    }
}
  
  
var getPokemonByWeakness = function(weakness) {

    var anyFound = false                                              //flag anyFound set to false
    var pokeArray = []                                                //initialized empty pokeArray

    for (var x in pokemonList.pokemon) {                              //iterate pokeon array
        for (var y in pokemonList.pokemon[x].weaknesses) {            //iterate through weakness array for each pokemon array
            if (pokemonList.pokemon[x].weaknesses[y] == weakness) {   //if iterated weakness matches entered weakness
   ...
   if (anyFound == true) {                                             //if anyFound set to true, Display the  pokeArray on the html page
        document.getElementById("id1").innerText = "The pokemon having weakness " + weakness + "-\n\n" + pokeArray.toString()
    } else {
        alert("No such pokemon found")                                  //if no result was found, alert No pokemon found
    }
...
// Entry point.User input, Choose option 1/2/3
var option = window.prompt("Select the option\n\n1)Pokemon Details by name\n\n2)Pokemon name in next Evolution\n\n3)Pokemon Names by Weakness")
  
  switch (option) {
  
    case "1":

      var enteredName = window.prompt("Enter the pokemon Name")       //user input
      getpokemonDetails(enteredName)                                  //call to getpokemonDetails function
      break
  
    case "2":
  
      var enteredName = window.prompt("Enter the pokemon Name")      //user input
      getpokemonEvolution(enteredName)                               // call to getpokemonEvolution function
      break
  
    case "3":
  
      var enteredWeakness = window.prompt("Enter the pokemon Weakness")   //user input
      getPokemonByWeakness(enteredWeakness)                               //getPokemonByWeakness
      break
  
    default:    
      alert("Invalid Option!")                                          //invalid input
  }
